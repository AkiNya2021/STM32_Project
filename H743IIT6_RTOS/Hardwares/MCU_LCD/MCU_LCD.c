...
void LCD_Init(void)
{
 ...
 if(lcddev.id!=0X9341)		    //非9341,尝试看看是不是NT35310
 {
   ...
   if(lcddev.id!=0X5310)		  //不是NT35310,尝试看看是不是NT35510
   {
     LCD_WR_REG(0XDA00);
     ...
     lcddev.id|=LCD_RD_DATA();		//读回0X00
     if(lcddev.id==0x8000)
       lcddev.id=0x5510;         //NT35510读回的ID是8000H,为方便区分强制设置为5510
     if(lcddev.id!=0X5510)			  //不是NT5510,尝试看看是不是SSD1963
     {
       LCD_WR_REG(0XA1);
       ...
       if(lcddev.id==0X5761)
         lcddev.id=0X1963;       //SSD1963读回的ID是5761H,为方便区分强制设置为1963
     }
   }
 }
 printf("LCD ID:%x\r\n",lcddev.id); //打印LCD ID
 ...
 if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)
 {
   ...
   FMC_Bank1E_R->BWTR[0]|=3<<0;	 	//地址建立时间(ADDSET)为3个HCLK =18ns
   FMC_Bank1E_R->BWTR[0]|=2<<8; 		//数据保存时间(DATAST)为6ns*3个HCLK=18ns
 }
